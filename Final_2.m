...
for id = 1:size(Ball_Matrix, 1)
    ...
    % x1=transl(end_position);
    % x2=transl([-3.2, 4, 0.23]);
    %
    ...
    % qMatrix(1,:) = kukarobot.model.ikcon(T,q0);                                            % Solve joint angles to achieve first waypoint
    %
    ...
    %     T = kukarobot.model.fkine(qMatrix(i,:)).T;                                           % Get forward transformation at current joint state
    ...
    %     J = kukarobot.model.jacob0(qMatrix(i,:));                 % Get Jacobian at current joint state
    %     m(i) = sqrt(det(J*J'));
    ...
    %         if qMatrix(i,j) + deltaT*qdot(i,j) < kukarobot.model.qlim(j,1)                     % If next joint angle is lower than joint limit...
    %             qdot(i,j) = 0; % Stop the motor
    %         elseif qMatrix(i,j) + deltaT*qdot(i,j) > kukarobot.model.qlim(j,2)                 % If next joint angle is greater than joint limit ...
    %             qdot(i,j) = 0; % Stop the motor
    %         end
    %     end
    ...
    %     kukarobot.model.animate(qMatrix(i,:));
    %     drawnow();
    % end
